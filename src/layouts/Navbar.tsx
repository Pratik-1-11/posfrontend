    ...
    <>
      <header className="fixed top-0 left-0 right-0 h-16 bg-slate-900 border-b border-white/5 flex items-center justify-between px-4 md:px-6 z-[50] font-inter no-print">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div className="flex items-center gap-2">
            <SyncIndicator />
            {user && ['VENDOR_ADMIN', 'VENDOR_MANAGER', 'CASHIER'].includes(user.role) && (
              <div className={`hidden md:flex items-center gap-2 px-3 py-1 rounded-full border ${isShiftOpen ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
                <div className={`w-1.5 h-1.5 rounded-full ${isShiftOpen ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`} />
                <span className="text-[10px] font-black uppercase tracking-wider">
                  Shift: {isShiftOpen ? 'Active' : 'Not Open'}
                </span>
                {isShiftOpen && (
                  <button
